...
### 1.1. 분산 시스템의 불확실성
앞선 [Saga Pattern 문서](./Saga-Pattern.md)에서 설명했듯이, 주문 서비스는 Saga 패턴을 이용해 분산 트랜잭션을 관리함.
...
*   **개선 방향:**
    1.  **타임아웃(Time-out) 단축:** 결제 대기 시간을 1시간에서 15분 내외로 단축하여 자원 회전율을 높임.
...
*   **개선 방향:** **Pagination(Cursor-based)** 처리를 도입하여 한 번에 처리하는 데이터 양을 조절하고, `(status, updatedAt)` 복합 인덱스를 통해 조회 성능을 최적화해야 함.
...
### 5.4 OrderSaga 기본키(PK) 문제
*   **문제점:** 현재 각 사가 엔티티는 PK가 UUID로 설정된 OrderSaga를 상속받아, UUID를 PK로 사용함. 이로 인해, 대량의 사가 데이터를 조회할 때 커서 기반 페이지네이션과 인덱싱 성능이 저하될 수 있음.
    1. **정렬 불일치:** UUID는 랜덤하게 생성되므로, 생성 순서와 정렬 순서가 일치하지 않아, 시간순 조회 시 별도의 정렬 작업(ORDER BY createdAt)이 필요함.
    2. **인덱스 단편화:** UUID 기반 PK는 인덱스 페이지 단편화를 유발하여, 대량 데이터 조회 시 디스크 I/O 증가 및 캐시 효율 저하로 이어질 수 있음.
    3. **유연성 부족:** 비즈니스 전략 변경으로 멱등성 보장 기준이 달라질 경우 (ex: UUID -> Snowflake), PK 변경이 어려워 확장성에 제약이 발생함.
*   **개선 방향:** 각 사가 엔티티에 별도의 `Long` 타입의 순차적 PK를 추가하여, 조회 및 페이징 성능을 향상시키는 방안을 고려해야 함.
    1. **AUTO_INCREMENT 활용:** MySQL의 AUTO_INCREMENT 기능을 사용하여 순차적 PK를 생성함.
    2. **TSID (Time-Sorted ID):** UUID의 고유성을 유지하면서 시간순 정렬이 가능한 ID 생성 방식 도입 (예: Twitter의 Snowflake 알고리즘).
       * 파티셔닝과 샤딩 전략까지 고려한다면, TSID 방식을 사용한 단일 PK가 가장 효율적인 방법이라고 생각함.
    3. **Unique 제약 조건:** 기존 멱등성 키(UUID) 필드를 PK가 아닌 Unique 제약 조건으로 변경하여, 고유성은 유지하되 조회 성능을 개선함.