...
### 7.3. 트랜잭션 가시성 부족
*   **문제점:** 현재는 DB 쿼리로만 상태를 확인해야 하므로, 여러 마이크로서비스를 오가는 복잡한 트랜잭션 흐름을 한눈에 파악하거나 병목 지점을 찾기 어려움.
*   **개선 방향:** **분산 추적(Distributed Tracing)** 시스템을 도입하여 트랜잭션의 전체 흐름과 구간별 소요 시간을 시각화해야 함.
