...
### 3.3. 타임아웃 및 타임리미터 설정 (TimeLimiter & Feign Timeout)

본 프로젝트는 OpenFeign을 이용한 **동기(Synchronous) 호출** 방식을 사용함. 동기 방식에서는 요청이 완료될 때까지 해당 스레드가 **블로킹(Blocking)** 되기 때문에, 단순히 실패 횟수뿐만 아니라 얼마나 오래 기다릴지에 대한 설정 또한 매우 중요함.

1.  **스레드 풀 고갈 방지:** 타임아웃이 없다면 지연되는 외부 서비스로 인해 주문 서비스의 모든 워커 스레드가 응답 대기 상태에 빠지게 되며, 이는 곧 신규 요청을 처리할 스레드가 없는 시스템 마비 상태로 이어짐.
2.  **빠른 실패와 자원 회수:** 적절한 타임아웃은 무의미한 기다림을 줄이고, 자원(스레드, 메모리)을 빠르게 회수하여 다른 정상적인 요청을 처리할 수 있게 함.
3.  **타임아웃 관련 설정**
    *   **Feign Client Timeout:** 네트워크 계층에서의 물리적 연결/읽기 제한.
...
| 설정 항목 | 설정값 | 설정 의도                                     |
| :--- | :--- |:--------------------------------------------|
| **failure-rate-threshold** | `50%` | 안정성과 민감도의 중간 지점으로 설정.                       |
| **sliding-window-size** | `10` | 적은 트래픽에서도 서킷 동작을 빠르게 테스트하기 위해 표본 크기를 작게 잡음. |
| **wait-duration-in-open-state** | `10s` | 외부 서비스의 복구 시간과 사용자 대기 시간을 고려함.              |
| **timeout-duration (TimeLimiter)** | `5s` | 5초 이상 응답이 없는 서비스는 비정상으로 판단하여 스레드 차단 방지.     |
| **feign.connectTimeout** | `2s` | 네트워크 연결은 2초 이내에 완료되어야 함.                    |
| **feign.readTimeout** | `5s` | 전체 비즈니스 타임아웃과 일치시켜 정합성 유지.     |

---

### 3.5. 대체 로직(Fallback) 구현
...
*   **개선 방향:** [Saga Pattern 문서의 모니터링 전략](./Saga-Pattern.md#73-트랜잭션-추적성-및-모니터링-강화)과 유사하게, Resilience4j의 **Micrometer** 연동을 통해 서킷 브레이커 메트릭을 수집하고, **Grafana 대시보드**에서 실시간 모니터링 및 알림 설정을 구축해야 함.
