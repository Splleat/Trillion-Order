...
본 프로젝트는 비동기 메시징(Kafka 등) 대신 **OpenFeign 기반의 HTTP 동기식 오케스트레이션 사가**를 채택함.
...
## 5. 주문 생성의 난제: 5xx 에러와 멱등성(Idempotency) 보장 전략
분산 시스템에서 가장 다루기 까다로운 문제는 외부 서비스 호출 시 발생하는 **Timeout**이나 **5xx (Internal Server Error)** 임.

### 5.1. 불확실한 상태
ex) 도서 서비스에 `재고 감소` 요청을 보냈는데 5xx 에러가 오거나 응답이 오지 않는 경우(Timeout), 클라이언트는 **재고 감소가 성공했지만, 응답을 받지 못한 것인지**, 아니면 **재고가 감소되지 않은 것인지** 알 수 없음.
...
### 6.3. 재시도 (Retry) 구현
주문 생성 사가가 실패 시 롤백(Rollback)하는 것과 달리, 취소 사가는 스케줄러로부터 **마지막 성공 지점(Checkpoint)** 부터 다시 시작함. 이를 통해 불필요한 중복 호출을 줄이고 최종적인 성공을 보장함.
...
> *   **Scheduler:** 시스템 장애 등으로 중단된 트랜잭션은 스케줄러가 감지하여 재시도함.
...
*   **동기 호출(Blocking I/O)의 처리량 한계:** Resilience4j Circuit Breaker를 도입하여 장애 전파는 차단했으나, HTTP 동기 호출 특성상 응답 대기 시간 동안 스레드가 점유되어 **시스템 전체 처리량(Throughput)** 이 제한되는 구조적 한계가 있음. (향후 메시지 큐 기반 비동기 Saga로 전환 고려)
*   **재시도 한계 설정:** 주문 취소 Saga의 무한 재시도는 시스템 리소스를 소모할 수 있음. 향후 **최대 재시도 횟수 설정** 및 실패 시 **관리자 알림** 기능을 추가할 예정임.
