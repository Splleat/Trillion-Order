...
public class MemberService {
    ...
    public void decreasePoint(UUID sagaId, Long memberId, Long orderId, int point) {
        memberClient.decreasePoint(sagaId, new PointUsageRequest(memberId, orderId, point));
    }

    // 포인트 적립 (주문 취소)
    @CircuitBreaker(name = "member-service", fallbackMethod = "fallbackIncreasePoint")
    @Retry(name = "member-service")
    public void increasePoint(UUID sagaId, Long memberId, Long orderId, int point) {
        memberClient.increasePoint(sagaId, new PointUsageRequest(memberId, orderId, point));
    }

    // 포인트 복구 (주문 생성 실패 시)
    @CircuitBreaker(name = "member-service", fallbackMethod = "fallbackRollbackPoint")
    @Retry(name = "member-service")
    public void rollbackPoint(UUID sagaId, Long memberId, Long orderId, int point) {
    ...
    public void fallbackIncreasePoint(UUID sagaId, Long memberId, Long orderId, int point, Throwable throwable) {
